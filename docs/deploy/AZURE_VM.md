# Deploying NimbusDB on an Azure VM (self-host profile, ADR-020)
...
- **A domain you control**, e.g. `example.com`. The platform will use:
  - `api.<domain>`, `console.<domain>`, `hosting.<domain>` — HTTPS via Traefik
  - `*.syd1.<domain>` — tenant database endpoints (Postgres TLS/SNI via pg-gateway)
...
```
api.<domain>        A  <VM_IP>
console.<domain>    A  <VM_IP>
hosting.<domain>    A  <VM_IP>
*.syd1.<domain>     A  <VM_IP>
```
...
vi /etc/nimbusdb/secrets.env      # set NDB_DOMAIN=<domain> and CLOUDFLARE_API_TOKEN=<token>
...
curl -X POST https://api.<domain>/v1/bootstrap -H 'Content-Type: application/json' \
  -d '{"bootstrap_token":"<NDB_BOOTSTRAP_TOKEN>","email":"you@example.com","org_name":"YourOrg"}'

# 2. console: https://console.<domain>  → sign in with the ndb_ key → New project
#    operator console: https://console.<domain>/admin  → sign in with NDB_ADMIN_TOKEN
...
psql "postgresql://<role>:<password>@ep-…​.syd1.<domain>:5432/<db>?sslmode=require"

# 5. scale-to-zero: suspend the branch in the console, reconnect — the first
#    connection wakes it (allow ~30 s connect timeout).
```

The in-console Knowledge Base (`https://console.<domain>/kb`) covers everything user-facing.
