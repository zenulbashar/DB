...
**Landed so far (read surface + connect + create/manage):** API-key connect flow (httpOnly cookie
session, server auth guard, sign-out), live projects dashboard, **project creation** (with the
one-time owner-credential reveal), project detail (branch list + per-branch endpoints + compute
bounds), the masked connection-string panel with one-click copy, and **branch management** (create +
suspend/resume/resize via server actions) — all against the live control plane through the one typed
client (ADR-012). The items below (magic-link sessions, SQL editor, metrics, role/database +
API-key/audit management UIs, read-replica endpoint creation) are the remaining Phase 3 slices.
