...
## ADR-001 · Product working name: "NimbusDB" — `proposed`
...
**Consequences:** rename is cheap by policy — no identifier in code embeds the name except DNS + token prefix (`ndb_`), both centralized. **Owner input requested.**

## ADR-002 · Go for control plane, gateway, and import engine — `proposed`
...
## ADR-003 · Kubernetes + CloudNativePG as the data-plane substrate — `proposed`
...
## ADR-004 · Two-generation serverless strategy: CNPG now, evaluate Neon OSS storage engine later — `proposed`
...
## ADR-005 · Substrate for `syd1` decided in Phase 1 (managed-k8s-first preference) — `proposed`
...
**Decision:** defer final pick to Phase 1 with a costed comparison; **default preference: managed k8s + cloud object storage/KMS** for launch, colo as a later cell (the cell model makes this placement, not migration).
**Consequences:** Terraform/module structure must stay provider-portable; CSI-snapshot capability is a hard requirement on any pick. **Owner input requested (budget envelope).**

## ADR-006 · One CNPG cluster per branch (no shared-instance tenancy for tenant data) — `proposed`
...
## ADR-007 · Custom `pg-gateway` for TCP ingress — `proposed`
...
## ADR-008 · Compute stays Nimbus's domain; NimbusDB does databases — `proposed`
...
## ADR-009 · Console adopts Nimbus design language; P2E handoff adopted as token/handoff format — `proposed`
...
**Consequences:** visual coherence with the hosting platform now; R-15 contained.

## ADR-010 · NATS JetStream for eventing; Temporal only from Phase 5 — `proposed`
...
## ADR-011 · PostgreSQL 17 default / 16 supported; pooled+direct dual endpoints are launch scope — `proposed`
...
## ADR-012 · Spec-first OpenAPI 3.1 + generated clients — `proposed`
...
## Open questions for the owner (blocking sign-off, not Phase 1 start)

| # | Question | Default if unanswered |
|---|---|---|
| Q1 | Product name — keep **NimbusDB**? (ADR-001) | NimbusDB |
| Q2 | Substrate budget envelope for `syd1` — managed cloud k8s OK at launch, or bare-metal/colo required? (ADR-005) | Managed k8s |
| Q3 | Was a dedicated **design export for this platform's console** intended? If yes, place per DESIGN_SYSTEM_MAPPING §5. | Proceed on Nimbus-derived interim system |
| Q4 | Billing processor preference for Phase 7 (Stripe assumed — both your apps already use Stripe/Resend stack)? | Stripe |
| Q5 | Migration order confirmation: Roster first, then Prompt2Eat? (MIGRATION_STRATEGY §7) | Roster first |
