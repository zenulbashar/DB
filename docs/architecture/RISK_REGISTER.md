...
| **R-2** | **Data loss on tenant databases** (bad backup, un-replayable WAL, silent corruption). Existential — this product's only non-negotiable. | **H** | 2+ | Continuous WAL archive + nightly base backups; **automated restore-verification now wired** (reconciler verify loop: ephemeral recovery clone per ready branch on an `NDB_VERIFY_INTERVAL` cadence, pass/fail ledger, failures surfaced on the admin console as a page — DATABASE_ARCHITECTURE §3); pre-destructive-op backups; second-region backup copies; restore drills as phase-gate items; synchronous replication on HA tier (**builder-enforced** since ADR-019: `preferred` durability + controlled switchover rendered into every HA-tier cluster). | Any verification failure; any storage-class change. |
...
| **R-16** | **Interim console auth is a long-lived API key in a cookie**, not a scoped session. Until email magic-link sessions land, a stolen `zdb_` key (broad scopes, long-lived) is the console credential; an XSS-exfiltrated or leaked key is high-impact. | M | 3 | Key stored **httpOnly** (no JS read) + `secure` in prod + `sameSite=lax`; console never renders the key back; secret reveal stays a separate audited `roles:write` API path (not the sign-in key); magic-link sessions with short TTL + rotation scheduled as the next Phase 3 slice to retire this. | Any leaked-key incident; magic-link sessions shipping (then retire). |
| **R-18** | **Single-VM self-host profile (ADR-020) is one failure domain** — VM loss takes the whole platform down until rebuilt; no multi-AZ, no substrate LB. Accepted consciously for the pre-GA budget. | M | self-host | WAL archives in MinIO are the durability net (continuous archiving + restore-verification loop); `secrets.env` backed up off-VM; the documented Binary Lane migration doubles as the rebuild/DR drill (`docs/deploy/BINARYLANE_MIGRATION.md`); DNS TTL 300 keeps cutover/rebuild fast; cloud profile (ADR-005) remains the GA path. | First paying external tenant (move to the cloud profile). |
| **R-17** | **Shared operator token (`NDB_ADMIN_TOKEN`)** is a single cross-tenant credential (ADR-018): a leak exposes every tenant's inventory and the lifecycle fix actions until rotated. | M→H | 3–7 | Surface disabled when unset; constant-time compare; token grants `/admin` only (no secret reads, no destructive ops in v1); every fix action lands in the affected tenant's audit log (`system`/`platform_admin`) so misuse is visible to tenants; admin console stores it httpOnly like the tenant key; rotate on staff change; superseded by operator RBAC + SSO/MFA in Phase 7 (then retire). | Any suspected leak (rotate immediately); operator RBAC shipping. |
